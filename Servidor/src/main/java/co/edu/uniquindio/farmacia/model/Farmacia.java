...
import co.edu.uniquindio.farmacia.exceptions.MedicamentoNoEncontradoException;
import co.edu.uniquindio.farmacia.exceptions.NoExisteDispensarioException;
import co.edu.uniquindio.farmacia.exceptions.PacienteNoEncontradoException;
...
public class Farmacia implements java.io.Serializable
{
	...
	public Medicamento buscarMedicamento (String codigo) throws MedicamentoNoEncontradoException
	{
		for (int contador=0; contador<medicamentos.size();contador++)
		{
			if(medicamentos.get(contador).getCodigo().equals(codigo))
			{
				return medicamentos.get(contador);
			}
		}
		throw new MedicamentoNoEncontradoException(codigo);
	}
	
	//Busca un paciente por cédula
	public Paciente buscarPaciente (String cedula) throws PacienteNoEncontradoException
	{
		for (int contador=0; contador<pacientes.size(); contador++)
		{
			if(pacientes.get(contador).getCedula().equals(cedula))
			{
				return pacientes.get(contador);
			}
		}
		throw new PacienteNoEncontradoException(cedula);
	}
	...
	//Verifica si el dispensario no existe
	public void verificarDispensarioVacio() throws NoExisteDispensarioException
	{
		if(this.dispensario==null)
		{
			throw new NoExisteDispensarioException();
		}
	}
